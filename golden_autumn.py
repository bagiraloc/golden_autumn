...
SHEET_URL = "https://docs.google.com/spreadsheets/d/1S5mf3gVU-FHgOJ_kpfTn02ZzYeXMw0VTfGNX-RL6KMY/edit?usp=sharing"
...
def save_data(df):
    df.to_csv("/mount/data/results_backup.csv", index=False)  # резервна копія на сервері
    st.warning("⚠️ Дані оновлюються тільки локально. Для синхронізації з Google потрібно GSheets API.")
